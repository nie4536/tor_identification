...
class deep_Data(Dataset):
    def __init__(self):
        data = pd.read_csv(r'E:\code\pretraining\deep_packet_input.csv', header=None)
        features = data.iloc[:,2:]
        labels = data.iloc[:,0]
        label_map = {label: index for index, label in enumerate(set(labels))}
        self.labels = list(map(lambda x: label_map[x], labels))
        # print(self.label)
        self.data = torch.tensor(np.array(features),dtype=torch.float)
...
import torchmetrics
model = torchkeras.KerasModel(Cnn,loss_fn=nn.CrossEntropyLoss(),metrics_dict = {"acc":torchmetrics.Accuracy(task='multiclass',num_classes=classNum)})
